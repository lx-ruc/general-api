<template>
  <article class="doc-article">
    ...
    <div class="doc-prose">
      ...
      <h2>计量计费</h2>
      <ul>
        <li><strong>计费公式</strong>：<code>单次消耗 = ceil((输入tokens × 输入单价 + 输出tokens × 输出单价) / 1M)</code>，
        ...
        <li><strong>双层记账</strong>：每次调用在同一个结算事务里累加子账号 <code>quota_used</code> 与客户
          <code>quota_used</code>，两级都是硬上限；结算在响应发出后无条件执行，超扣幅度封顶在单请求消耗内。</li>
        ...
        <li><strong>缓存命中不计费</strong>：开启精确缓存后，相同请求命中缓存直接返回
          （响应头 <code>X-Tg-Cache: hit</code>），记录用量日志但消耗为 0。</li>
      ...
      <p>客户维护受控中心词表（如「AI 客服」「数据分析」），API key 挂靠中心；结算时把中心快照写进调用日志——
        改派只影响未来，历史账单不可变。客户报表按中心聚合，未归集的用量单独披露占比。</p>
      ...
      <ul>
        ...
        <li>调用日志只记计量元数据（模型、token、消耗、渠道），不落对话内容。</li>
      </ul>
    </div>
  </article>
</template>
