...
- **API 转换**：对外暴露统一 OpenAI 兼容端点（`/v1/chat/completions`、`/v1/embeddings`、`/v1/models`），聊天支持 SSE 流式；用户只需把 SDK 的 `base_url` 指向慧沐引擎
- **API 路由**：一个 API key 可调用多个模型（请求中的 `model` 参数决定路由），能调哪些模型由客户管理员授权
- **模型名映射**：渠道可按模型配置「对外名 → 上游名」，换渠道、灰度、别名切换客户零改码；出站请求改写为上游名，响应（含流式每块）自动改写回对外名，计费与缓存锚定对外名，映射对客户完全不可见
...
- **渠道快速接入**：新建渠道时直接粘贴厂商控制台复制的 curl 示例（或裸 URL），前端本地解析出 Base URL、端点路径、密钥与模型名自动填表，创建后自动探活验证连通
...
- **定时渠道体检**：周期探活启用中的渠道（`channel_test_interval`，建议 30m），连续失败达阈值自动禁用（可被自动探测拉起）并邮件告警——低流量哑渠道的故障不再依赖业务流量触发熔断才被发现
- **管理面访问令牌**：平台/客户管理员可在「修改密码 → 访问令牌」签发 `tgp_` 令牌（SHA-256 落库、明文仅显示一次、可设有效期/随时吊销），用于程序化对接管理 API（开户/授权/查用量等）；权限与登录账号完全一致（RBAC + org 隔离），吊销/账号变动即时生效
