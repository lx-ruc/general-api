...
- **单月上限与欠费停服**：公司/员工可设单月消费上限（token 预算，0=不限），当月达限拦截（429 `monthly_limit_exceeded`）、次月自动清零恢复（结算路径原子重置，无需定时任务）；公司总额度耗尽自动转「欠费停服」（数据面 403 `insufficient_balance`，管理台仍可登录），平台追加额度即自动恢复——与手动停用互不干扰；公司开户支持联系人/联系电话
